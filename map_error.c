...
/*   Updated: 2022/04/02 11:29:27 by ahamdy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void	check_invalid_conponents(char **map, int bonus)
{
	int	i;
	int	j;
	...
	while (map[i])
	{
		...
		while (map[i][j])
		{
			check_characters(map, i, j, bonus);
			j++;
		}
		i++;
	}
...
void	check_map_error(char **map, size_t y, int bonus)
{
	...
	check_invalid_conponents(map, bonus);
}
