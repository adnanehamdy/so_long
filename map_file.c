...
/*   Updated: 2022/03/31 10:17:57 by ahamdy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	open_file(char *filename)
{
	...
	if (!ft_strncmp(&filename[ft_strlen(filename) - 4], ".ber", 4))
	{
		...
		if (fd == -1)
			error_exit("check the file name or permissions\n", 1);
	}
	else
	{
...
char	**convert_file_to_string(char *filename, int bonus)
{
	int		fd;
	char	**map;
	int		map_lines;
	...
	check_map_error(map, map_lines, bonus);
	return (map);
}
